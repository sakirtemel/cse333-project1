...
int checkCommands(char **commands){
	...
	proc.outputFilename = NULL;
	proc.inputFilename = NULL;
	proc.appendRedirect = 0;
	proc.isBackground = 0;
	...
	if (proc.outputFilename != NULL){
		//check append
		saveOut();
		if(proc.appendRedirect==1)
			_redirectOutput(proc.outputFilename,1);
		else
			redirectOutput(proc.outputFilename);
		redirectOut = 1;
	}
	if (proc.inputFilename != NULL){
		//check append
		saveIn();
	//	if(proc.appendRedirect==1)
		//	_redirectOutput(proc.outputFilename,1);
		//else
			redirectInput(proc.inputFilename);
		redirectIn = 1;
	}
